...
void ImGUILayerVulkan::Init(const ImGuiCreateInfo& createInfo) {
    ...
    if (std::holds_alternative<VulkanContextInfo>(createInfo.backendData)) {
        ...
        ImGui_ImplVulkan_LoadFunctions(context.apiVersion, load_vk_func, &instance);
        ...
        pipline_rendering_ci.setColorAttachmentCount(1).setColorAttachmentFormats(context.colorFormat);

        initInfo.PipelineRenderingCreateInfo = pipline_rendering_ci;
